...
# "Selenium," is a powerful open-source framework for automating web browsers. It allows you to control web browsers programmatically, automate interactions with web applications, and perform various web-related tasks using code. You can use Selenium to open web browsers (e.g., Chrome, Firefox, Edge) and interact with them as if a human user were using the browser. This includes actions like clicking buttons, filling out forms, and navigating between web pages. i.e webBrowser automation
...
# to prevent the unwantined warning in terminal by selenium
...
try:
    ...
    chrome_driver_path = 'C:\\Users\\Dell\\OneDrive\\Desktop\\python\\Jarvis\\Brain\\chromedriver.exe'
    chrome_options = Options()
    # chrome_options.headless = False #!note
    chrome_options.add_argument("--headless=new")
    chrome_options.add_experimental_option('excludeSwitches', ['enable-logging'])
    chrome_options.add_argument('--log-level=3')
    service = Service(chrome_driver_path)
    # Add the options to disable UI pop-ups and use fake media devices
    ...
    driver.maximize_window() #maximaze the window
    driver.get(url)
...
while True:
    ...
    if len(text) == 0:
        ...
        output_file_path = "C:\\Users\\Dell\\OneDrive\\Desktop\\python\\Jarvis\\Body\\SpeechRecognition.txt"
